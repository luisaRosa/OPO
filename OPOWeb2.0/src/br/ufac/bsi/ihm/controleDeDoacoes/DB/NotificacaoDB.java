...
public class NotificacaoDB {
	...
	public int addNotificacao(Notificacao notificacao) {
		...
		String QueryUpdate = "INSERT INTO notificacao(status, dt_notificacao, horaNotificacao, usuarioSistema_cpf)"
				+ " VALUES('" + notificacao.getStatus() + "', '" + notificacao.getDtNotificacao() + "', '"
				+ notificacao.getHoraNotificacao() + "','" + notificacao.getUsuario().getCpf() + "');";

		return conexao.atualize(QueryUpdate);

	}

	public int updNotificacao(Notificacao notificacao) {
		String UpdateQuery = "UPDATE notificacao SET " + "status = '" + notificacao.getStatus() + "' WHERE id ="
				+ notificacao.getId() + ";";
	...
	public Notificacao getNotificacao(int id) throws SQLException, ParseException {
		...
		if (rs != null) {
			...
			if (rs.next()) {
				...
				notificacao.setHoraNotificacao(rs.getString("horaNotificacao"));
